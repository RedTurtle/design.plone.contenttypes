...
class IRicevutaPagamento(model.Schema):
    ...
    stampa_ricevuta = field.NamedFile(
        title=_(u'stampa_ricevuta', default=u'Stampa ricevuta'),
        required=True,
    )

    data_pagamento = schema.Date(
        title=_(u'data_pagamento', default=u'Data pagamento'),
        required=True,
    )

    importo_pagato = schema.TextLine(
        title=_(u'importo_pagato', default=u'Importo pagato'),
        required=True,
    )
    ...
    pratica_associata = field.NamedFile(
        title=_(u'pratica_associata', default=u'Pratica associata al pagamento'),
        required=True,
    )

    # TODO: capire se ci sono altri esiti oltre ai banali "accettato", "rifiutato", "in attesa di conferma", 
    ...
    allegato = field.NamedFile(
        title=_(u'allegato', default=u'Allegato'),
        required=False,
    )
