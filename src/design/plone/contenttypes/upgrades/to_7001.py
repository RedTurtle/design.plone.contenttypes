...
def create_incarico_for_persona(context):
    ...
    for brain in brains:
        ...
        if safe_hasattr(persona, "compensi-file") and safe_hasattr(incarico, "compensi-file"):
            compensi = getattr(persona, "compensi-file")
            for obj in compensi.contentItems():
                api.content.move(
                    source=obj,
                    target=incarico["compensi-file"]
                )

        if safe_hasattr(persona, "importi-di-viaggio-e-o-servizi") and safe_hasattr(incarico, "importi-di-viaggio-e-o-servizi"):
            importi = getattr(persona, "importi-di-viaggio-e-o-servizi")
            for obj in importi.contentItems():
                api.content.move(
                    source=obj,
                    target=incarico["importi-di-viaggio-e-o-servizi"]
                )
